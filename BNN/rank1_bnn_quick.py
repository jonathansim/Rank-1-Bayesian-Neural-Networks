...
def main():
    ...
    if args.use_subset:
        ...
        run_name = f"QuickRun_M{args.ensemble_size}_B{batch_size}_D{args.rank1_distribution}_NumSamp{args.num_eval_samples}" 
        # M for ensemble size, B for batch size, S for scheduler
    
    wandb.init(project='rank1-bnn-WR-quick', mode=mode_for_wandb, name=run_name)
